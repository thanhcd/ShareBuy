import { View, Text, SafeAreaView, TouchableOpacity, Image, TextInput, Keyboard, TouchableWithoutFeedback } from 'react-native'
...
const phone = () => {
  ...
  const handleSavePhone = async () => {
    ...
    if (!/^[0]\d{8}$/.test(trimmedPhone)) {
        alert("Số điện thoại không hợp lệ! Số điện thoại phải có 9 chữ số và bắt đầu bằng số 0.");
        return;
    }

    try {
        await updateProfileField(userIdAuth, "Phone", trimmedPhone);
